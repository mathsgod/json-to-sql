...
class JsonToSql
{
    ...
    public function generateCreateTableStatement(array $table): string
    {
        ...
        foreach ($foreignKeys as $foreignKey) {
            $columnDefinitions[] = $this->generateForeignKey($foreignKey);
        }
     ...
     * 生成 FOREIGN KEY
     */
    private function generateForeignKey(array $foreignKey): string
    {
        $columns = $foreignKey['columns'] ?? [];
        $referenceTable = $foreignKey['reference_table'] ?? '';
        $referenceColumns = $foreignKey['reference_columns'] ?? [];

        if (empty($columns) || empty($referenceTable) || empty($referenceColumns)) {
            throw new Exception('Foreign key requires columns, reference_table, and reference_columns');
        }

        $columnList = implode('`, `', $columns);
        $referenceColumnList = implode('`, `', $referenceColumns);

        $def = "  FOREIGN KEY (`{$columnList}`) REFERENCES `{$referenceTable}` (`{$referenceColumnList}`)";

        // 處理 ON UPDATE
        $onUpdate = $foreignKey['on_update'] ?? null;
        if ($onUpdate) {
            $def .= " ON UPDATE " . strtoupper($onUpdate);
        }

        // 處理 ON DELETE
        $onDelete = $foreignKey['on_delete'] ?? null;
        if ($onDelete) {
            $def .= " ON DELETE " . strtoupper($onDelete);
        }
